...
curs=conn.cursor()  #veritabanı ile diyalog kurma
sorguCreTblSpor=("CREATE TABLE IF NOT EXISTS spor(             \
                    ...
                    MHal TEXT NOT NULL,                               \
                    Kilo TEXT NOT NULL)")
curs.execute(sorguCreTblSpor) #oluşturduğumuz sorguları yazacağımız kısım
conn.commit()
...
def EKLE():
    ...
    curs.execute("INSERT INTO spor   \
                     ...
                     VALUES (?,?,?,?,?,?,?,?,?)", \
                     (_lneTCK,_lneSporcuAdi,_lneSporcuSoyadi,_cmbSporKulubu,_lwBrans,\
                      _cmbCinsiyet,_cwDTarihi,_medeniHal,_spnKilo))
    conn.commit()
    LISTELE()
...
def LISTELE():
    ...
    ui.lblKKayitSayisi.setText(str(kayitSayisi[0]))
...
def SİL():
    ...
    if cevap == QMessageBox.Yes:
        ...
        try:
            ...
            ui.statusbar.showMessage("Hata : " + str(Hata))
...
#--------------------------------------------------------
def GUNCELLE():
    cevap = QMessageBox.question(penAna,"GÜNCELLE","Kaydı güncellemek istediğinize emin misiniz?",\
                         QMessageBox.Yes | QMessageBox.No)
    ...
    if cevap==QMessageBox.Yes:
        try:
            ...
            curs.execute("UPDATE spor SET TCNo=?, SporcuAdi=?,SporcuSoyadi=?, Kilo=?, \
                        ...
                        (_lneTCK,_lneSporcuAdi,_lneSporcuSoyadi,_spnKilo,_cmbSporKulubu,_lwBrans,\
                        _cmbCinsiyet,_cwDTarihi,_medeniHal, _Id) )
            
            conn.commit()
            LISTELE()

        except Exception as Hata:
            ui.statusbar.showMessage("Hata mesajı: "+ str(Hata))
...
ui.menHakkinda.triggered.connect(HAKKINDA)
